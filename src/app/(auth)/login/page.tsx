  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        setIsSubmitted(true);
        toast.success("¡Enlace mágico enviado con éxito!");
      } catch (err) {
  ...
  return (
    <Card className="w-full border-neutral-800/80 bg-neutral-900/60 shadow-2xl backdrop-blur-xl shadow-black">
      <CardHeader className="space-y-1 text-center">
        <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-amber-400/10 text-amber-400 ring-4 ring-amber-400/5">
          <Sparkles className="h-6 w-6 animate-pulse" />
        </div>
        <CardTitle className="bg-gradient-to-r from-amber-400 to-amber-200 bg-clip-text text-3xl font-extrabold tracking-tight text-transparent">
          Iniciar Sesión
        </CardTitle>
        <CardDescription className="text-neutral-400">
          Ingresa tu email para recibir un enlace de acceso mágico y sin contraseña.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isSubmitted ? (
          <div className="flex flex-col items-center justify-center py-6 text-center space-y-4 animate-in fade-in zoom-in duration-300">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-neutral-400 max-w-sm">
                Hemos enviado un enlace de inicio de sesión único a{" "}
                <span className="font-semibold text-amber-400">{email}</span>. Haz clic
                en el enlace para acceder de forma instantánea.
              </p>
            </div>
            <Button
              variant="ghost"
              className="text-xs text-amber-400 hover:text-amber-300 hover:bg-neutral-800/50 mt-4"
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <Input
                  ...
                  disabled={isPending}
                  className="pl-11 h-12 bg-neutral-950/70 border-neutral-800 text-white placeholder-neutral-500 rounded-xl focus-visible:ring-amber-400/20 focus-visible:border-amber-400 transition-all"
                  required
                />
              </div>
              {errors.email && (
              ...
              {errorParam && (
                <p className="text-xs font-semibold text-rose-500 pl-1 text-center">
                  Error de autenticación. Por favor, solicita un nuevo enlace.
                </p>
              )}
            </div>
            <Button
              type="submit"
              disabled={isPending}
              className="w-full h-12 rounded-xl bg-amber-400 text-neutral-950 hover:bg-amber-300 font-bold text-sm tracking-wide shadow-lg shadow-amber-400/20 hover:shadow-amber-400/30 transition-all flex items-center justify-center gap-2 group cursor-pointer"
            >
              {isPending ? (
                <>
      ...
      <CardFooter className="flex flex-col items-center space-y-4 border-t border-neutral-800/40 pt-6">
        <p className="text-xs text-neutral-500 text-center">
          ...
          <Link
            href={`/registro${nextRoute ? `?next=${encodeURIComponent(nextRoute)}` : ""}`}
...
export default function LoginPage() {
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 overflow-hidden bg-black py-12">
      ...
      <div className="absolute bottom-[-20%] right-[-10%] w-[60%] h-[60%] rounded-full bg-amber-400/10 blur-[120px] pointer-events-none" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] h-[350px] rounded-full bg-amber-500/5 blur-[90px] pointer-events-none" />
