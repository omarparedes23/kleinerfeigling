...
> = {
  ...
  "green lemon": {
    bg: "bg-lime-950/30 backdrop-blur-md",
    text: "text-lime-400",
  ...
  default: {
    ...
    glow: "shadow-[0_0_15px_rgba(163,230,53,0.05)]",
...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    <div className={`flex w-full gap-3 ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      {/* Avatar */}
      <div
        className={`flex h-9 w-9 shrink-0 select-none items-center justify-center rounded-full border text-xs font-black transition-all duration-300 ${
          isUser
            ? "border-lime-500/30 bg-lime-950/30 text-lime-400 shadow-[0_0_10px_rgba(163,230,53,0.2)]"
            : "border-purple-500/30 bg-purple-950/30 text-purple-400 shadow-[0_0_10px_rgba(168,85,247,0.2)]"
        }`}
      >
      ...
      <div className="flex flex-col gap-2.5 max-w-[85%] md:max-w-[75%]">
        {/* Main Text Content */}
        {message.content && (
          <div
            ...
            <CardContent className="p-3.5 flex flex-col justify-between min-h-[110px]">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <Button
                  ...
                  className="h-8 rounded-lg bg-lime-400 text-neutral-950 font-bold text-xs hover:bg-lime-300 border-none transition-all shadow-[0_0_10px_rgba(163,230,53,0.2)] cursor-pointer"
                >
                  Agregar
                </Button>
              </div>
  ...
  return (
    <Card
      className={`border backdrop-blur-md transition-all duration-300 ${
        isAvailable
          ? "border-lime-500/20 bg-lime-950/10 shadow-[0_0_10px_rgba(163,230,53,0.05)]"
      ...
      <CardContent className="p-3.5 flex items-start gap-3">
        <div
          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
            isAvailable ? "bg-lime-500/15 text-lime-400" : "bg-rose-500/15 text-rose-400"
          }`}
        >
  ...
  return (
    ...
    >
      <CardContent className="p-3.5 flex items-start gap-3">
        <div
          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
            isAvailable ? "bg-lime-500/15 text-lime-400" : "bg-amber-500/15 text-amber-400"
          }`}
        >
        ...
        <div className="flex-1 space-y-1">
          <h4 className="font-bold text-xs text-neutral-200">
            Cálculo de Envío: <span className="text-lime-400">{args.distrito}</span>
          </h4>
          ...
          {isAvailable && (
            <div className="flex gap-4 mt-2 border-t border-neutral-900/60 pt-2 text-[10px] text-neutral-400">
              ...
              <div>
                <span>Tiempo: </span>
                <span className="font-bold text-lime-400">{result.tiempo}</span>
              </div>
            </div>
          )}
        </div>
...
function BuscarRecetaCard({ result }: { result: any }) {
  ...
  return (
    <div className="flex flex-col gap-3.5">
      <div className="text-[11px] font-bold text-lime-400/80 uppercase tracking-widest px-1">
        {result.mensaje}
      </div>
...
function AgregarAlCarritoCard({
  ...
  return (
    <Card className="border-lime-500/30 bg-lime-950/10 backdrop-blur-md shadow-[0_0_15px_rgba(163,230,53,0.1)]">
      <CardContent className="p-3.5 flex items-center gap-3">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-lime-400 text-neutral-950 shadow-[0_0_10px_rgba(163,230,53,0.3)]">
          <Check className="h-5 w-5 stroke-[3px]" />
        </div>
        <div className="flex-1 space-y-0.5">
          <h4 className="font-black text-xs text-lime-400 font-sans tracking-wide">
            ¡AGREGADO CON ÉXITO!
          </h4>
